...
st.markdown("~~ Work in progress. Last updated 24th / April / 22.~~")
...
st.write("Note: The source code for this page can be found at https://github.com/spyderroque/Georgia-Ukraine-War/tree/Nightly- .")
...
st.write("Population data as given by the state of Georgia [8] are given in red, while the estimations of the UN [9] are given in blue. Unlike the UN data these data do not include estimates for Abkhazia and South Ossetia. Abkhazia was as a matter of fact never controlled by the Georgian state and South Ossetia is missing in the estimates from 2008 onward (i.e. census of 2014.) The markings indicate values gained in a census. Between 1991 and the mid 90s Georgia was occupied with its secession from the USSR / Russia and then years of civil war.")
...
fig.update_layout(title = "Population over time", title_font_size = 22, legend_orientation = 'h', legend_yanchor = "top", legend_y = 1.1)
...
st.write("Click or double click on the legend or drag left click on the graph to select specific elements or manipulate the graph.")

fig10 = go.Figure()
#fig10.add_trace(go.Scatter(x=df_PopReg.index.values, y=df_PopReg['Georgia']*1000,
...
for value in range(1,len(dicttemp)):
    fig10.add_trace(go.Scatter(x=df_PopReg.index.values, y=df_PopReg[dicttemp[value]]*1000,
                    mode='lines',fill='tonexty', name=dicttemp[value], stackgroup='one'))
fig10.update_layout(title = "Population by region (Areaplot)", title_font_size = 22,height = 750, legend_orientation = 'h', legend_yanchor = "top", legend_y = 1.05)
st.plotly_chart(fig10, use_container_width=True)


st.write("Especially in the case of Abkhazians it is quite visible, that most Abkhzians left Georgia in favor of living in Abkhazia. Click or double click on the legend or drag left click on the graph to select specific elements or manipulate the graph.")
...
fig20.update_layout(title="Ethnic composition", title_font_size=22,legend_orientation = 'h', legend_yanchor = "top", legend_y = 1.1)
...
fig30 = go.Figure()
fig30.add_trace(go.Bar(y = df_PopAgeSex.index.values, x = df_PopAgeSex.loc[idx[:,(2021, "Males")]], 
                       name = 'Male', orientation = 'h'))
fig30.add_trace(go.Bar(y = df_PopAgeSex.index.values, x = (-1)*df_PopAgeSex.loc[idx[:,(2021, "Females")]], 
                       name = 'Female', orientation = 'h'))

# Updating the layout for our graph
fig30.update_layout(title = 'Population Pyramid of Georgia (2021)',
...
#fig30.add_hrect(y0=29, y1=46, line_width=0, fillcolor="yellow", opacity=0.3)
fig30.add_hline(y=46, line_width = 2, line_dash = "dash", line_color = "black")
fig30.add_hline(y=29, line_width = 2, line_dash = "dash", line_color = "white")
fig30.update_layout(legend_orientation = 'h', legend_x = 0.45, legend_yanchor = "bottom", legend_y = -0.15)
...
st.write('[7] https://images.pexels.com/photos/4550542/pexels-photo-4550542.jpeg?cs=srgb&dl=pexels-genadi-yakovlev-4550542.jpg&fm=jpg')
st.write('[8] https://geostat.ge/media/38040/01---population-by-self-governed-unit.xlsx')
st.write('[9] http://data.un.org')
st.write('[10] http://census.ge/files/results/english/17_Total%20population%20by%20regions%20and%20ethnicity.xls')
st.write('[11] https://en.wikipedia.org/wiki/Demographics_of_Georgia_(country) on 02/04/2022')
st.write('[12] ')
st.write('[13] ')
st.write('[14] ')
st.write('[15] ')
